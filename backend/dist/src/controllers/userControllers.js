...
import { findUserAndDelete, findUserById, getUsers, isExist, updateBanStatusById, updateRoleStatusById, } from '../services/userServices.js';
...
export const resetPassword = async (req, res, next) => {
    try {
        ...
        const decoded = jwt.verify(token, dev.jwt.reset_k);
        if (!decoded) {
            next(ApiError.unauthorized('Invaild token'));
            return;
        }
        const updatedPassword = await User.findOneAndUpdate({ email: decoded.email }, { $set: { password: bcrypt.hashSync(password, 10) } });
        if (!updatedPassword) {
...
export const unadminUser = async (req, res, next) => {
    try {
        const id = req.params.userId;
