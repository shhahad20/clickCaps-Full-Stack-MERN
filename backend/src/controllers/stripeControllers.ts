...
export const checkoutOrder = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const line_items = req.body.orders.orderItems.map((item: OrderItem) => {
...
interface CartItem {
  ...
  price: number;
  quantity: number;
  // Add any other properties if present in your cart items
}

export const stripeCheckout = async (req: Request, res: Response, next: NextFunction) =>{
  const line_items = (req.body.cartItem as CartItem[]).map(item => {
    return{
      price_data:{
        currency:'usd',
        product_data:{
