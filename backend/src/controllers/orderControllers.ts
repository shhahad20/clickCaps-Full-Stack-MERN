...
export const createOrder = async (
  ...
  try {
    ...
    if (userInfo) {
      ...
      const emailToSend = OrderCreatedEmail(
        userInfo.email,
        userInfo.first_name,
        userInfo.last_name,
        totalAmount
...
export const updateOrder = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!updatedOrder || !currentOrder) throw Error('No such order found')

    const users = await User.find({ _id: { $in: updatedOrder.user } })

    users.map(async (user) => {
      const emailToSend = OrderUpdatedEmail(
        user.email,
        user.first_name,
        user.last_name,
        updatedOrder.status
...
export const updateOrderByUser = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const id = req.params.id
    const data = req.body
